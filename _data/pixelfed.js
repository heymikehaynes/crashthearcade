...
async function downloadAndOptimizeImage(imageUrl, postTitle) {
	try {
		...
		const imageDirectory = path.join(__dirname, "..", "public", "img", "pixelfed");
...
module.exports = async function () {
	let rssUrl = "https://pixelfed.social/users/crashthearcade.atom"; // Replace with your RSS feed URL
	...
	try {
		...
		const posts = await Promise.all(feed.items.slice(0, 6).map(async (item) => {
